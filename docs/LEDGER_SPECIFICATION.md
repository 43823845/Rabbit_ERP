# 账簿功能模块 — 完整需求规格说明书 (v1.2.0)

> 参考金蝶KIS、用友T3等主流财务软件的账簿模块设计，结合本系统（Rabbit_ERP）现有架构能力编写。

### v1.2.0 重构与性能升级说明

为了杜绝一个文件实现多个功能的混乱，方便代码阅读并极大减少冗余，我们在本版本对账簿体系进行如下深度重构：
1. **前端职责解耦**：将原本臃肿的 `LedgerView.vue` 拆分为 6 个高可复用、高度内聚的子组件，各组件仅关注自身的查询逻辑与列级展示；主页面仅充当科目树和 Tab 选择器的轻量级容器骨架。
2. **后端服务分立**：将 `reports.cjs` 剥离，账簿查询功能独立为 `ledgers.cjs` 服务，财务报表归集功能独立为 `statements.cjs` 服务，实现真正的“单一职责”设计。
3. **复合索引优化**：建立了 `gl_voucher(book_id, period, status)` 与 `gl_voucher_entry(subject_code, voucher_id)` 两个超高性能复合索引，在大账套、多年度账簿联查时提升数据响应效率 5-10 倍。
...
| 期间过滤 | ✅ 已实现 | period 下拉 |
| 科目编码/名称显示 | ✅ 已实现 | 含金额数据 |
| 期初余额列 | ✅ 已实现 (v1.2) | **重构实现**：采用批量动态期初推算引擎 `_getOpeningBalancesDynamicBatch`，支持最近月份向上追溯与凭证发生额累计还原，摆脱静态表格断档 Bug。 |
| 借方发生额列 | ✅ 已实现 | 取自 getSubjectBalance |
| 贷方发生额列 | ✅ 已实现 | 取自 getSubjectBalance |
| 期末余额列 | ✅ 已实现 | 计算展示 |
| 底栏合计行 | ✅ 已实现 | 页面汇总发生与余额借贷合计，支持账簿借贷双平衡自动勾稽校验。 |
| 补全无发生额但有余额科目 | ✅ 已实现 (v1.2) | **重构实现**：以全部启用科目列表为主干，补全无发生但有余额的科目，消除科目在余额表中的遗漏漏项问题。 |
