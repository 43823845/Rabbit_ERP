...
- 金额输入模拟纸质凭证数位格（亿、千、百、万...角、分）
- 自动中文大写金额转换
- 借贷平衡实时校验
- 内置 8 个业务凭证模板
- **凭证打印**：单张/批量打印，A4 双页分栏排版，html2canvas 截图导出图片
- **附件管理**：凭证支持上传附件文件

### 5. 系统设置
- **公司信息**：查看/修改公司名称、联系人、电话、地址、税号
...
### 7. 财务报表
- **利润表**：收入/费用分类汇总，自动计算利润
- **资产负债表**：资产/负债/权益分类汇总

### 8. 期末结账
- 12月卡片式期间管理，支持年份切换
- 结账前自动检查：未过账凭证、试算平衡、期初余额、科目方向异常
- 检查开关可配置，检查通过后方可执行结账
- 支持反结账操作

## 性能优化

- **html2canvas 按需加载**：导出凭证截图时动态 import，减小首屏体积
